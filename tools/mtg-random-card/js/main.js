window.i18n.initPromise.then(() => {
	...
	document.getElementById("filterForm").addEventListener("submit", async function (e) {
		...
		if (document.getElementById("wifiConfirm")?.checked !== true) {
			showToast(window.i18n.t('dataUsageRequired'));
			return;
		}
		...
		if (usedPT) {
			...
			if (!userSelectedCreature) {
				...
				showToast(window.i18n.t('creatureTypeAdded'));
			}
		...
		try {
			...
			if (!imgURL) {
				throw new Error(window.i18n.t('noImage'));
			}
			...
			// 필터 요약
			const summary = [];
			if (typeList.length > 0) summary.push(`<strong>${window.i18n.t('summaryTypes')}</strong> ${typeList.join(", ")}`);
			if (colorList.length > 0) summary.push(`<strong>${window.i18n.t('summaryColors')}</strong> ${colorList.join(", ")}`);
			if (rarityList.length > 0) summary.push(`<strong>${window.i18n.t('summaryRarity')}</strong> ${rarityList.join(", ")}`);
			if (form.querySelector("#excludeBasic")?.checked === true) summary.push(`<strong>${window.i18n.t('summaryExcludeBasic')}</strong>`);
			if (cmcRaw !== "") summary.push(`<strong>${window.i18n.t('summaryCmc')}</strong> ${form.querySelector("#cmcOperator")?.value} ${cmcRaw}`);
			if (powerOp && powerVal) summary.push(`<strong>${window.i18n.t('summaryPower')}</strong> ${powerOp} ${powerVal}`);
			if (toughOp && toughVal) summary.push(`<strong>${window.i18n.t('summaryToughness')}</strong> ${toughOp} ${toughVal}`);
			if (usedPT && numericPTOnly) summary.push(`<strong>${window.i18n.t('summaryPT')}</strong> ${window.i18n.t('numericOnly')}`);
			...
			img.onerror = () => { throw new Error(window.i18n.t('imageLoadFailed')); };
		} catch (err) {
			cardResultDiv.innerHTML = `<p style="color:#ff5555;">${window.i18n.t('error')} ${err.message}</p>`;
			console.error(err);
		}
	});
